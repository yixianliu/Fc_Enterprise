...
window.ShearPhoto.prototype = {
    transformFun: function () {
        var Imgstyle = document.body.style,
                arr = ["MsTransform", "MozTransform", "WebkitTransform", "WebkitTransform", "OTransform", "transform"];
                ...
                if (leng > 0) {
                    ...
                    this.domimg = [domimg[0], leng], this.close_ = function () {
                        for (var i = 0; i < leng; i++) {
                            ...
                            if ("cssText" in domimgi.style)
                                domimgi.style.cssText = "";
                            else
                    ...
                    this.SetPreview = function (p) {
                        if (p) {
                            for (var i = 0; i < leng; i++)
                                domimg[1][i].style.height = Math.round(arg.preview[i] / p) + "px";
                            proportionFun = EmptyFun;
                        } else
                            proportionFun = function (d, w, i, pro) {
                                d[1][i].style.height = Math.round(w[i] / pro[0] * pro[1]) + "px";
                            };
                    };
    ...
    config: function (arg) {
        ...
        if (navigator.userAgent.indexOf("MSIE 6.0") > 0 && arg.Border === 0)
            this.ie6 = function (a, b, c) {
    ...
    zoom: function () {
        ...
        zoom.run({
            ...
            MoveFun: function (L) {
                var schedule;
                if (L < bisect)
                    schedule = Math.round(Zoomout * L + 10) / 100;
                else
                    schedule = Math.round(L * magnify - 100) / 100;
    ...
    defaultShear: function () {
        ...
        if ("cssText" in this.arg.ImgMain.style)
            this.arg.ImgMain.style.cssText = this.arg.ImgDom.style.cssText = "";
        else {
...
window.ShearPhoto.MoveDiv = function () {
    this.arg = [], this.ReckonWH = this.DivW = this.DivH = this.selectionempty = this.addevent = this.DivDownEVe = this.DomMoveEve = this.DomUpEve = this.eveMold = false;
};
...
window.ShearPhoto.MoveDiv.prototype = {
    ...
    DomMove: function (this_, dom, disX, disY, formLeft, formTop) {
        var argform = this_.arg.form,
                ...
                shifting = this_.arg.shifting = Object.prototype.toString.call(this_.arg.shifting) === "[object Array]" && this_.arg.shifting.length > 1 ? this_.arg.shifting : [0, 0],
